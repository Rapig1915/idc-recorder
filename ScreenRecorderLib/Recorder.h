...
namespace ScreenRecorderLib {
	...
	public:
		...
		/// <summary>
		///Video Output Size, 0*0 : keep original size
		/// </summary>
	...
	// Screen Recorder class for Display Recording
	...
	// Record option and data structs for game recording
	public ref class GameProcessInfo {
	public:
		property int ProcessId;
		property String^ WindowTitle;
		property String^ ExeFilePath;
		
		GameProcessInfo() {

		}
		GameProcessInfo(int processId, String^ windowTitle, String^ exeFilePath) {
			ProcessId = processId;
	...
	// Game Recoder class for recording game windows
	public ref class GameRecorder
	{
	public:
		static List<GameProcessInfo^>^ LoadGameList();
		static int ShotGame(String^, String^);
		static int LoadD3DEngine();

		GameRecorder();
		int Start(String^ processExeName, String^ videoPath, RecorderOptions^ options);
		int Stop();
		void SetOptions(RecorderOptions^ options);
		int GetCaptureStatus();

		property bool IsRecording;
		property String^ m_VideoPath;
	private:
		property RecorderOptions^ m_RecoderOptions;
		property String^ m_CurrentRecordingGame;
	};
}

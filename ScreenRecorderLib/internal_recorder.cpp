...
void internal_recorder::SetVideoSize(UINT32 width, UINT32 height)
{
	m_OutputWidth = width;
...
HRESULT internal_recorder::InitializeVideoSinkWriter(std::wstring path, _In_opt_ IMFByteStream *pOutStream, _In_ ID3D11Device* pDevice, RECT sourceRect, RECT destRect, DXGI_MODE_ROTATION rotation, _Outptr_ IMFSinkWriter **ppWriter, _Out_ DWORD *pVideoStreamIndex, _Out_ DWORD *pAudioStreamIndex)
{
	...
	*pAudioStreamIndex = 0;
	...
	/*UINT destWidth = min(sourceWidth, max(0, m_OutputWidth));
	UINT destHeight = min(sourceHeight, max(0, m_OutputHeight));
	if (destWidth == 0)		destWidth = max(0, destRect.right - destRect.left);
	if (destHeight== 0)		destHeight = max(0, destRect.bottom - destRect.top);*/
